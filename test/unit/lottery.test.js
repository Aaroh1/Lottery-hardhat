    ...
    : describe("Lottery Unit Tests", function () {
          ...
          describe("enter", function () {
              it("reverts when you don't pay enough", async () => {
                  try{await lottery.enter() // is reverted when not paid enough or lottery is not open
                  assert(false);
                }catch(e)
                {assert(e)}
              })
              it("records player when they enter", async () => {
                  await lottery.enter({ value: lotteryEntranceFee })
                  const contractPlayer = await lottery.getPlayer(0)
                  assert.equal(player.address, contractPlayer)
              })
              it("emits event on enter", async () => {
                  await expect(lottery.enter({ value: lotteryEntranceFee })).to.emit( // emits lotteryEnter event if entered to index player(s) address
                      lottery,
                      "EnterLottery"
                  )
              })
              it("doesn't allow entrance when lottery is calculating", async () => {
                  ...
                  //----------------------------------
                  //Increase time by interval+1 inorder to invoke performUpkeep and change value of Lottery state
                  //--------------------------------
                  await network.provider.request({ method: "evm_mine", params: [] })
                  // we pretend to be a keeper for a second
                  await lottery.performUpkeep([]) // changes the state to calculating for our comparison below
                  await expect(lottery.enter({ value: lotteryEntranceFee })).to.be.revertedWith( // is reverted as lottery is calculating
          ...
          describe("checkUpkeep", function () {
              it("returns false if people haven't sent any ETH", async () => {
                  ...
                  await network.provider.request({ method: "evm_mine", params: [] })
                  /* -----------------------------------------------------------------
                  callStatic helps in simulating calls to a txn 
                  callStatic is a useful method that submits a state-changing transaction to an Ethereum node, but asks the node to simulate the state change, rather than to execute it. 
          ...
          describe("fulfillRandomWords", function () {
            ...
            // All the assertions are done once the WinnerPicked event is fired
              it("picks a winner, resets, and sends money", async () => {
                  ...
                  for (let i = startingIndex; i < startingIndex + additionalEntrances; i++) { // i = 1; i < 4; i=i+1
